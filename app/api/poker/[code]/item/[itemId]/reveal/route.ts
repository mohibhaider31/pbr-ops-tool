...
export async function POST(_req: Request, { params }: { params: { code: string; itemId: string } }) {
  ...
  await prisma.pokerItem.update({ where: { id: params.itemId }, data: { state: "REVEALED" } });
  await pusher().trigger(pokerChannel(params.code), POKER_EVENTS.revealed, { itemId: params.itemId });
