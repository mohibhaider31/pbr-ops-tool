  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      <header className="flex items-end justify-between gap-6 px-[30px] pt-6 pb-4 border-b border-border">
        <div className="flex flex-col gap-[5px]">
          ...
          <p className="m-0 text-[12.5px] text-muted">
            Engine → Middleware → Frontend handoffs across active stories. Click a cell to advance it.
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-4 gap-px bg-border border-b border-border">
        {[
          { label: "ACTIVE STORIES", value: stats.total, note: "non-terminal" },
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden">
        <div
          ...
          style={{ gridTemplateColumns: "64px minmax(150px,1fr) 150px 150px 150px 120px" }}
        >
        ...
        {visible.length === 0 && (
          <div className="p-8 text-sm text-muted2 text-center">
            {onlyStalls ? "Nothing stalled or blocked right now." : "No active stories in the pipeline."}
          </div>
        )}
        ...
        {visible.map((row) => {
          ...
          return (
            <div
              key={row.jiraKey}
              className="grid gap-0 px-[30px] items-center min-h-[52px] py-2 border-b border-borderLight hover:bg-cream transition-colors"
              style={{ gridTemplateColumns: "64px minmax(150px,1fr) 150px 150px 150px 120px" }}
            >
        ...
        <div className="px-[30px] py-[18px] pb-10">
          <span className="text-[12px] text-muted2">
            Cells cycle: — → In progress → Done → Blocked. Stall flags fire immediately when a layer is done but the next hasn&apos;t started.
          </span>
        </div>
      </div>
